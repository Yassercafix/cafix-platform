...
-- but the application code previously inserted status = 'free' which does not exist in the enum.
-- This migration aligns the DB enum with the current application code.
-- ============================================================================
...
ALTER TYPE "public"."table_status" ADD VALUE IF NOT EXISTS 'available';
...
-- Step 2: Update the default value for the status column to 'available'
ALTER TABLE "cafeteriaTables" ALTER COLUMN "status" SET DEFAULT 'available';

-- Step 3: Migrate existing rows that use old enum values to new ones
UPDATE "cafeteriaTables" SET "status" = 'available'   WHERE "status" = 'free';
